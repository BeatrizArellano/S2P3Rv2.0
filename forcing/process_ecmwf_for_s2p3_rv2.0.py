...
domain_file = '../sims/NW_Eur/s12_m2_s2_n2_h_map_NW_Eur.dat'
...
directory_containing_files_to_process = '/data/local_ssd/ra499/sims/NW_Eur/all_constant/met_data/ecmwf/'
output_directory = '/data/local_ssd/ra499/sims/NW_Eur/all_constant/met_data/processed/'
...
def land_fill(mask_cube,cubes):
    ...
    where_mask = mask_cube.data > 0.5  ### Value for the ERA-20C dataset
    where_mask = np.flipud(where_mask[0])
...
input_variables = ['vas','uas','clt','hurs','tas','psl','rsds','rlds','wind_speed'] #these are the cmip5 names, and should all be avaiable daily NOTE WE HAVE huss not hurs... so need to convert huss to hurs/ wind speed is calculated in this script
...
if not exists:
    print('Creating wind_speed file')
...
for year in range(start_year,end_year+1):
    ...
    for k in range(len(input_variables)):
        ...
        cube.data = np.flip(cube.data,axis=1)
    ...
    znew[:,np.where(input_variables2 == 'rsds')[0],:] /= (60.0*60.0*24) ## The Era20C forecast data is in J/m2 accumulated after 06:00UTC + step hours (In this case 24 hours)
    znew[:,np.where(input_variables2 == 'rlds')[0],:] /= (60.0*60.0*24)	### See https://www.ecmwf.int/en/forecasts/datasets/reanalysis-datasets/era-20c 
    ...
    for u,longitude_point in enumerate(sample_points_lat_lon['lon'].values):
        ...
        long_point = longitude_point if longitude_point>=0 else 360 + longitude_point
        np.savetxt(output_directory+output_filename+'lat'+str(np.round(latitude_point,4))+'lon'+str(np.round(long_point,4))+'_'+str(year)+'.dat', df2[['day_number','wind_speed','wind_direction','clt','tas','psl','hurs','rsds','rlds']].values, fmt='%s%10.2f%10.2f%10.2f%10.2f%10.2f%10.2f%10.2f%10.2f')
